...
for page in range(page_start, page_end):
    ...
    gui_text['bg'] = '#10d269'
    gui_label_1['text'] = '暂无数据'
    gui_label_2['text'] = '暂无数据'
    ...
    try:
        if tbPageVersion == 0:
            ...
            for i, goods in enumerate(goods_arr):
                gui_label_1['text'] = f'正在获取第{i}个,共计{goods_length}个'
                ...
                goods_item = {"商品名称": item_name, "商品价格": item_price, "月销售量": month_deals,
                              "归属地": ships_from, "商品店铺名称": item_shop, "商品链接": item_link}
            ...
            for i, goods in enumerate(goods_arr):
                ...
                item_link = goods.find_element(By.CSS_SELECTOR,
                    ...
                    'href')
                goods_item = {"商品名称": item_name, "商品价格": item_price, "月销售量": month_deals,
                              "归属地": ships_from_province + ' ' + ships_from_city, "商品店铺名称": item_shop,
                              "商品链接": item_link}
                output_list += [goods_item]
    except:
        gui_text['text'] = f'本页面定位元素失败，程序将于5秒后重新尝试爬取'
        gui_text['bg'] = 'red'
        gui_label_2['text'] = '暂无信息'
        gui_label_1['text'] = f'注意:第【{page}】页将跳过如需获取请重新运行程序！'
    ...
    for delay in range(delay_time):
        ...
        gui_label_2['text'] = f'等待下次翻页{delay}秒，总共需等待{delay_time}秒'
        time.sleep(1)
...
output_dataframe.to_excel(
    '淘宝爬取商品结果销售量排序' + f'{time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())}' + '.xlsx',
    index=False)
