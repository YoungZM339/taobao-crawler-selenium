...
with open(f'./taobao_{datetime.date.today()}.csv', 'w+', encoding='utf-8', newline='') as files:
    # 设置表头并写入csv文件
    csv_obj = csv.DictWriter(files, fieldnames=['商品名称', '商品价格', '店铺名称', '店铺地址', '购买人数', '商品链接'])
    csv_obj.writeheader()

    profile_dir = r'--user-data-dir=C:\Users\youngzm\AppData\Local\Google\Chrome\User Data'
    # 加载配置数据
    ...
    search_keyWord = driver.find_element(By.ID, 'q').send_keys('冰红茶')
